<template>
  <div class="index" v-loading.fullscreen.lock="fullscreenLoading" element-loading-text="拼命加载中...">
    ...
    <a id="downlink"></a>
    <el-input v-model="input" placeholder="请输入对象名称，比如: Person  Example 等 首字母大写"></el-input>
    <el-button class="button" @click="uploadFile()">导入</el-button>
    <el-button class="button" @click="downloadFile(excelData)">导出</el-button>
    <el-button class="button" @click="update()" type="danger" v-if="currentCql">提交</el-button>
    <!--错误信息提示-->
    ...
    <!--展示导入信息 https://segmentfault.com/q/1010000008780378-->
    ...
    <el-table :data="excelData" style="width: 100%">
      <el-table-column v-for="col in cols" v-bind:key="col.prop" :prop="col.prop" :label="col.label" align="center">
      </el-table-column>
    </el-table>
  </div>
</template>
    ...
    methods: {
        ...
        if (data.length <= 0) {
          ...
          if (data.length > 0 ) {
              for (var prop in data[0]) {
                  // create(:A{name:'1'}),(:A{name:'2'}),(:A{name:'3'})
                  console.log('obj.'+ prop + ' = ' + data[0][prop])
                  var tmp = {}
                  tmp['prop'] = prop
                  tmp['label'] = prop
                  this.cols.push(tmp)
              }
              this.createCql(this.excelData)
      ...
      update() {
          if (this.currentCql != '' && this.input != '' ) {
              Http.getCql({"cql":this.currentCql}).then(data=>{
                  console.log(data)
                  if (data.status == 200) {
                      this.currentCql = ''
                      this.excelData = []
                      ...
                      this.$message({
                          message: '提交失败' + data.data,
                          type: 'error'
                      })
                  }
              })
          } else {
              this.$message.error('未导入数据或对象名称为空')
          }
      },
